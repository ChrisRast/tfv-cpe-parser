...
interface StringifyOptions {
  binding?: typeof uriBindingPrefix | typeof formattedBindingPrefix;
}
...
export class CpeParser {
  ...
  stringify(model: Cpe, options?: { prefix: 'formatted' | 'uri' }): string {
    const values = [
      ...
      .map((value) => value && this.formatAttributeValue(value))
      .join(':');

    if (options?.prefix) {
      const binding =
        options?.prefix === 'formatted'
          ? formattedBindingPrefix
          : uriBindingPrefix;
  ...
  private getAttributeList(fullCpe: string, cpeAttributes: string): string[] {
    ...
    if (
      !this.hasUriBinding(fullCpe) ||
      !fullCpe.includes(uriBindingExtendedAttributesDelimiterKey)
    )
      return attributes;
  ...
  public hasUriBinding(cpe: string): boolean {
    return (
      Boolean(cpe) &&
      typeof cpe === 'string' &&
      cpe.startsWith(uriBindingPrefix)
    );
  }
  ...
  public hasFormattedBinding(cpe: string): boolean {
    return (
      Boolean(cpe) &&
      typeof cpe === 'string' &&
      cpe.startsWith(formattedBindingPrefix)
    );
  }
